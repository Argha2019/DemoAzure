...
class CustomerViewOrder extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
			showModal: false,
			...
			loading: false,
			agentId: encryptByAES(JSON.parse(decryptByAES(Cookies.get("customerData")))[0].id),
			orderId: "",
			orderPlacedDate: "",
			orderAmount: "",
			orderStatus: "",
	...
	componentDidMount() {
		...
		var url = Config.serverUrl + '/ordersearch/ordersearch/ordersByCustomerId';
		//var url ="http://192.168.43.26:9229/ordersearch/ordersByCustomerId";
		axios.put(url, {"customerId" : this.state.agentId})
	...
	getProductDetail() {
		...
		this.setState({ loading: true }, () => {
			this.state.productList.map(p => {
				promises.push(
					axios.get(Config.serverUrl + '/products/products/getProduct/' + p.productId)
				);
			});
			axios
				.all(promises)
				...
				<div className="container-fluid">
					<div style={{ margin: "5%" }}>
						{/*<div className="buttonLeft">
							<Button onClick={this.showMoreDetails}></Button>
							<br />
							<br />
						</div>*/}
						<div
