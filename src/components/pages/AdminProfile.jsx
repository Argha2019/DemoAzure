...
export default class AdminProfile extends React.Component {
	constructor(props) {
		...
		let profileData = {};
		try {
			profileData = JSON.parse(decryptByAES(Cookies.get('adminData')))[0];
		}
		catch (e) {
			profileData = {
				firstName: '',
	...
	submitProfile(e) {
		...
		if (formValid(this.state)) {
			let user = {
				"id": encryptByAES((JSON.parse(decryptByAES(Cookies.get('adminData')))[0].id)),
				...
				"status": JSON.parse(decryptByAES(Cookies.get('adminData')))[0].status,
				"password": encryptByAES("Abcde@123"),
				"accessToken": JSON.parse(decryptByAES(Cookies.get('adminData')))[0].accessToken,
				"refreshToken": JSON.parse(decryptByAES(Cookies.get('adminData')))[0].refreshToken,
				"expireTime": JSON.parse(decryptByAES(Cookies.get('adminData')))[0].expireTime
			}
			...
			axios.put(url, user)
				.then(
					response => {
						...
						Cookies.set('adminData', encryptByAES(JSON.stringify([user])));
